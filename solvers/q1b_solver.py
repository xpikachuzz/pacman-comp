...
def astar_initialise(problem: q1b_problem):
    ...
    state = State(0, [pac_pos], reachable_food)
...
def astar_loop_body(problem: q1b_problem, astarData: AStarData):
    ...
    min_state = astarData.open.pop()        # O(1)

    # O(food). does this have food? has this food been visited before? 
...
def astar_heuristic(current, goals = []):
    # Return the minimum to the heuristics to all the goals. O(food)
    small = float('inf')
...
def h_dist(coord1, coord2):
    ...
    return util.manhattanDistance(coord1, coord2)
